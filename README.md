...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remix-hydrogen
  fly apps create remix-hydrogen-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-hydrogen
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-hydrogen-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remix-hydrogen-staging' does not match app name in config file 'remix-hydrogen'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remix-hydrogen-db
  fly postgres attach --app remix-hydrogen remix-hydrogen-db

  fly postgres create --name remix-hydrogen-staging-db
  fly postgres attach --app remix-hydrogen-staging remix-hydrogen-staging-db
  ```
